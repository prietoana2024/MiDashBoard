...
const createDataToTransactionResume = (transactions) => {
  ...
  return {
    approvedTransactions: {
      count: approvedTransactions.length,
      totalAmount: approvedTransactions.reduce((sum, t) => sum + t.totalAmount, 0)
    },
    canceledTransactions: {
      count: canceledTransactions.length,
      totalAmount: 0
...
const Transactions = () => {
  ...
  const requestExcel = () => {
    ...
    let body = {
      transactionIds: filteredTransactions.map(t => t.id),
      paypadId: selectedPaypad.id,
      fileName: `Reporte_${formatPaymentTypeLabel(selectedPaymentType) || "Todos"}_${selectedPaypad.username}_${formatDate(dateRange.from)}_a_${formatDate(dateRange.to)}.xlsx`.replace(" ", "")
    };
