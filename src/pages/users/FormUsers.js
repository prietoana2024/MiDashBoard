...
//SEGUNDO CODIGO COMENTADO EL FUNCIONAL
/*
...
const FormUsers = ({
    ...
    if (idUserToEdit !== null) {
      userService
        ...
        .then(({ response }) => {
          ...
          // If we're editing a user, also fetch their assigned offices
          officeService
            ...
            .then(({ response }) => {
              ...
              // If the user has a client, also fetch all offices for that client
  ...
  const handleFormChange = ({ target }) => {
    if (target.localName === "select") {
      ...
      // Handle client selection - load associated offices
      if (target.name === "client") {
        const clientId = Number(target.selectedOptions[0].id);
        loadClientOffices(clientId);
        setSelectedOffices([]); // Reset selected offices when client changes
      }
  ...
  const handleOfficeSelection = (officeId) => {
    ...
    // For debugging only, properly log the updated selection
    // Use the updated array instead of string concatenation
    console.log("Updated selected offices:", selectedOffices.filter(item => item !== id).concat(selectedOffices.includes(id) ? [] : [id]));
  };
  return (
    <>
      <h2 className="mx-3 mt-4 fs-4">
...
export default FormUsers;*/import React, { useEffect, useState } from "react";
import PropTypes from "prop-types";
import userDto from "../../Dto/usersDto";
import userService from "../../services/userService";
import mastersService from "../../services/mastersService";
